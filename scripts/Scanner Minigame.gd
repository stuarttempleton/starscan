...
@export var success_curve: Curve
...
func _ready():
	...
	var bgMargin = -bg.position.x
	oscillatorRange = bg.size.x - bgMargin * 2
	oscillatorRange = bg.size.x
...
func _process(delta):
	...
	if not scanButtonPressed:
		...
		oscillator.position.x = oscillatorRange * abs(x - 1.0) + bg.position.x
...
func _on_Button_pressed():
	...
	var score = success_curve.sample(abs(x - 1.0))
...
func SetupSweetSpot():
	...
	sweetSpot.position.x = oscillatorRange * leftMin.x + bg.position.x
	sweetSpot.size.x = oscillatorRange * (rightMin.x - leftMin.x)
